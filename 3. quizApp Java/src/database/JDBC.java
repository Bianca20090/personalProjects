...
public class JDBC {
    private static final String DB_URL = "jdbc:mysql://127.0.0.1:3306/quizsc";
    private static final String DB_USERNAME = "root";
    private static final String DB_PASSWORD = "pass123word*";

    public static boolean insertQuestionToDBB(String question, String category, String[] variante, int correctIndex) {
        try {
            Connection connection = DriverManager.getConnection(DB_URL, DB_USERNAME, DB_PASSWORD);

            Category categorieObj = getCategory(category);
            if (categorieObj == null) {
                categorieObj = insertCategory(category);

            }

            //inserare intrebare in baza de date

            Question questionObj = insertQuestion(categorieObj, question);

            return insertAnswers(questionObj, variante, correctIndex);
    ...
    private static boolean insertAnswers(Question question, String[] variante, int indexCorect) {
        try {
            Connection connection = DriverManager.getConnection(DB_URL, DB_USERNAME, DB_PASSWORD);
            ...
            for(int i=0;i<variante.length;i++){
                insertAnswerQuery.setString(2, variante[i]);
                if(i==indexCorect){
    ...
    public static ArrayList<Question> getQuesstions(Category category){
    ...
    public static ArrayList<Answer> getAnswers(Question question){
        ...
        try {
            ...
            PreparedStatement getanswersQuery=connection.prepareStatement("SELECT * FROM QUESTION JOIN ANSWER" +
                    " ON QUESTION.QUESTION_ID = ANSWER.QUESTION_ID " +  // Adaugă un spațiu aici
